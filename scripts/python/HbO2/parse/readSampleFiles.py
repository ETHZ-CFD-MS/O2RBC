...
def createSampleDirPath(caseName, setDirectory):
    dirPath = os.path.join(caseName, 'postProcessing', setDirectory)
    if not os.path.isdir(dirPath):
        warnings.warn('Directory {:s} not found. Trying same path without ''postProcessing/'''.
                      format(dirPath))
        dirPath = os.path.join(caseName, setDirectory)
    return dirPath
...
def createSampleFilePath(caseName, setDirectory, time, setName, fieldNames=['PO2']):
    ...
    dirPath = createSampleDirPath(caseName, setDirectory)
    pathToSample = os.path.join(dirPath, timeString,
