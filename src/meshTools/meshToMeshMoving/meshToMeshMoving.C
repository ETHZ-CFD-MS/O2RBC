...
namespace Foam
{
    defineTypeNameAndDebug(meshToMeshMoving, 0);

    template<>
    const char* Foam::NamedEnum
    <
        Foam::meshToMeshMoving::interpolationMethod,
        5
    >::names[] =
    {
        "direct",
        "mapNearest",
        "cellVolumeWeight",
        "cellRelativeVolumeWeight",
        "cellRelativeVolumeWeightCartesian"
    };

    const NamedEnum<meshToMeshMoving::interpolationMethod, 5>
        meshToMeshMoving::interpolationMethodNames_;
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

Foam::labelList Foam::meshToMeshMoving::maskCells
(
    const polyMesh& src,
    const polyMesh& tgt
) const
...
void Foam::meshToMeshMoving::correctWeights()
{
    // normalize weights only for direct, mapNearest and cellVolumeWeight
    if (method_ == imDirect ||
        method_ == imMapNearest ||
        method_ == imCellVolumeWeight)
    {
        normaliseWeights
        (
            "source",
    ...
    // for cellRelativeVolumeWeight, clamp weights so their total per cell
    // is less than or equal to one. This is required when cells from a given
    // mesh overlap.
    else if (method_ == imCellRelativeVolumeWeight)
    {
        clampWeights
        (
            "source",
...
void Foam::meshToMeshMoving::normaliseWeights
(
...
) const
{
    ...
    {
        nCell = returnReduce(wght.size(), sumOp<label>());
    }
    else
    {
        nCell = wght.size();
...
void Foam::meshToMeshMoving::clampWeights
(
    const word& descriptor,
    const labelListList& addr,
    scalarListList& wght
...
void Foam::meshToMeshMoving::calcAddressing
(
    const polyMesh& src,
    const polyMesh& tgt
)
{
    autoPtr<myMeshToMeshMethod> methodPtr
    (
        myMeshToMeshMethod::New
        (
...
void Foam::meshToMeshMoving::calculate()
{
...
//        (void)newTgt.cellTree();
        ...
        (
            ...
            false,
            tgtToSrcCellAddr_,
            reductionOperations::ListPlusEqOp<label>(),
            flipOp(),
            labelList()
        );
        ...
        (
            ...
            false,
            tgtToSrcCellWght_,
            reductionOperations::ListPlusEqOp<scalar>(),
            flipOp(),
            scalarList()
        );

        // correct the interpolation weights
        correctWeights();
    ...
    else
    {
        ...
        // Info<< "End: Calculating addressing..." << endl;
...
Foam::meshToMeshMoving::interpolationMethodAMI
(
...
) const
{
    switch (method_)
    {
        ...
        case imCellRelativeVolumeWeight:
        {
            return AMIPatchToPatchInterpolation::imFaceAreaWeight;
            break;
        }
        case imCellRelativeVolumeWeightCartesian:
        {
            return AMIPatchToPatchInterpolation::imFaceAreaWeight;
            break;
        }
...
Foam::meshToMeshMoving::patchAMIs() const
{
    if (patchAMIs_.empty())
    {
...
Foam::meshToMeshMoving::meshToMeshMoving
(
    const polyMesh& src,
    const polyMesh& tgt,
    const interpolationMethod& method,
    const bool interpAllPatches,
    const bool serialInterp
)
:
    srcRegion_(src),
    tgtRegion_(tgt),
...
void
Foam::meshToMeshMoving::update()
{
    // if the number of cells in either mesh changed, clear the addressing and
    // weights.
    if (   srcRegion_.nCells() != srcToTgtCellAddr_.size()
        || tgtRegion_.nCells() != tgtToSrcCellAddr_.size())
    {
        if (debug)
        {
            Info<< "Foam::meshToMeshMoving::update(): "
                << "Number of cells changed, clearing the addressing and "
                << "weight lists." << endl;
        }
        srcToTgtCellAddr_.clear();
        srcToTgtCellWght_.clear();
        tgtToSrcCellAddr_.clear();
        tgtToSrcCellWght_.clear();
    }
