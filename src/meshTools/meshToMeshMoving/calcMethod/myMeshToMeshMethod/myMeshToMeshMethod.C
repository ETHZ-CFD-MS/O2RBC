...
namespace Foam
{
    defineTypeNameAndDebug(myMeshToMeshMethod, 0);
    defineRunTimeSelectionTable(myMeshToMeshMethod, components);
}

Foam::scalar Foam::myMeshToMeshMethod::tolerance_ = 1e-6;

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

Foam::labelList Foam::myMeshToMeshMethod::maskCells() const
{
...
bool Foam::myMeshToMeshMethod::intersect
(
...
Foam::scalar Foam::myMeshToMeshMethod::interVol
(
    const label srcCellI,
    const label tgtCellI
) const
...
void Foam::myMeshToMeshMethod::appendNbrCells
(
    const label cellI,
    const polyMesh& mesh,
...
bool Foam::myMeshToMeshMethod::initialise
(
...
) const
{
    ...
    if (!src_.nCells())
    {
        // set the elements in target-to-source addressing to zero to prevent
        // previous interpolation results from being re-used.
    ...
    {
        // set the elements in source-to-target addressing to zero to prevent
        // previous interpolation results from being re-used.
...
Foam::myMeshToMeshMethod::myMeshToMeshMethod
(
    const polyMesh& src,
    const polyMesh& tgt
)
...
void Foam::myMeshToMeshMethod::writeConnectivity
(
