...
namespace Foam
{
    defineTypeNameAndDebug(myCellVolumeWeightMethod, 0);
    addToRunTimeSelectionTable
    (
        myMeshToMeshMethod,
        myCellVolumeWeightMethod,
        components
    );
}
...
bool Foam::myCellVolumeWeightMethod::findInitialSeeds
(
...
) const
{
    ...
    const pointField& cellCenters = src_.cellCentres();
    ...
    for (label i = startSeedI; i < srcCellIDs.size(); i++)
    {
        ...
        if (mapFlag[srcI])
        {
            pointField pts(srcCells[srcI].points(srcFaces, srcPts).xfer());
            // also use the cell center for more robust seed finding when the
            // source and target meshes have their points on the same planes
            pts.append(cellCenters[srcI]);
...
void Foam::myCellVolumeWeightMethod::calculateAddressing
(
...
void Foam::myCellVolumeWeightMethod::setNextCells
(
    label& startSeedI,
    label& srcCellI,
    label& tgtCellI,
...
Foam::myCellVolumeWeightMethod::myCellVolumeWeightMethod
(
    const polyMesh& src,
    const polyMesh& tgt
)
:
    myMeshToMeshMethod(src, tgt)
{}
...
void Foam::myCellVolumeWeightMethod::calculate
(
