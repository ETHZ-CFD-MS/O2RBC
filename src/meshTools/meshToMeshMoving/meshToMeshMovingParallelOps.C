...
Foam::label Foam::meshToMeshMoving::calcDistribution
(
    const polyMesh& src,
    const polyMesh& tgt
) const
{
    ...
    if (Pstream::parRun())
    {
        ...
        if (nHaveCells > 1)
        {
            ...
            if (debug)
            {
                Info<< "meshToMeshMoving::calcDistribution: "
...
Foam::label Foam::meshToMeshMoving::calcOverlappingProcs
(
...
Foam::autoPtr<Foam::mapDistribute> Foam::meshToMeshMoving::calcProcMap
(
    const polyMesh& src,
    const polyMesh& tgt
) const
...
void Foam::meshToMeshMoving::distributeCells
(
...
void Foam::meshToMeshMoving::distributeAndMergeCells
(
    const mapDistribute& map,
    const polyMesh& tgt,
    const globalIndex& globalI,
